...
{
    int x = Determine_the_type(av); //if int= 1; float= 2; double= 3; char= 4; else 0
    std::string text = av;
    if (x == 1)
    {
        if (_int > 31 && _int < 127)
...
bool ScalarConverter::isint(char* charPtr)
{
    ...
    if ((charPtr[0] == '-' || charPtr[0] == '+') && std::isdigit(charPtr[1]))
        charPtr++;
    std::strtol(charPtr, &endptr, 10); // base 10 (decimal)
...
int ScalarConverter::Determine_the_type(char *str)
{
    std::string text = str;
    if (isint(str))
    {
